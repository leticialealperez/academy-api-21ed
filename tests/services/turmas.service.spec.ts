...
describe('Testes para a classe TurmasService', () => {
    ...
    it('Cadastrar turma - Deve retornar um erro quando existir uma turma cadastrada com o mesmo valor de programa e edição', async () => {
    ...
    it('Cadastrar turma - Deve ser cadastrado a nova turma quando informado algo diferente de uma edição e programa previamente cadastrado', async () => {
        // 1 - Configurar os inputs e cenário
        const input: CadastrarTurmaDto = {
            edicao: 21,
            programa: 'Starter Dev Full Stack',
    ...
    test('Listar turmas - Deve retornar a lista de turmas totais quando não enviado nenhum filtro', async () => {
    ...
    test('Listar turmas - Deve retornar a lista de turmas correspondentes ao filtro de edição', async () => {
        const input = {
            edicao: 21
        }
        ...
        const listaFiltrada = turmasMock.filter((t) => t.edicao === input.edicao)

        prismaMock.turma.findMany.mockResolvedValue(listaFiltrada);


        // 2 - Invocar a função
        const result = await sut.listar(input);
